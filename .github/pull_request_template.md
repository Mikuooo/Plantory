## Intent

Describe the user-visible or engineering outcome and link the approved source.

## Scope

- Changed boundaries:
- Explicitly unchanged:
- Design or execution plan:

## Ownership

- [ ] Matching CODEOWNERS were notified, or the author is the owner and recorded self-review
- [ ] Owner feedback, unavailable owners, and unresolved ownership risk are recorded
- [ ] Ownership was used for review routing only; required approval was not assumed

## Risk

- Data, compatibility, security, or native risk:
- Rollback:

## Verification

- [ ] `pnpm harness:check`
- [ ] Original product path revalidated where behavior changed
- [ ] Adjacent shared behavior checked
- [ ] Android/iOS evidence attached or unverified platform disclosed

Commands and evidence:

## Repository Knowledge

- [ ] Architecture, commands, routes, schemas, and environment documentation remain accurate
- [ ] New review guidance was encoded in docs or an executable check when durable
- [ ] Reviewed documents append a date in `docs/quality/document-freshness.json`, or no registered document was reviewed
- [ ] Post-merge defects are recorded in `docs/quality/escaped-defects.json`, or this change is not an escaped-defect repair
